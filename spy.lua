...
local function _D(s)local r=""for i=1,#s do r=r..string.char(string.byte(s,i)-1)end return r end
...
_G.BoatFly=false
_G.FullBright=false
task.spawn(function()while true do if _G.BoatFly then pcall(function()local z=game.Players.LocalPlayer.Character local A=z and z.Humanoid.SeatPart if A and A:IsA("VehicleSeat")then local B=A:FindFirstChild("SpyGyro")or Instance.new("BodyGyro",A)B.Name="SpyGyro"B.MaxTorque=Vector3.new(math.huge,math.huge,math.huge)if _G.BoatSpeed>0 then A.CFrame=A.CFrame*CFrame.new(0,0,-(_G.BoatSpeed/35))end A.CFrame=CFrame.new(A.Position.X,_G.BoatHeight,A.Position.Z)*A.CFrame.Rotation A.Velocity=Vector3.new(0,0,0)end end)end
task.wait()end end)
task.spawn(function()local C=game:GetService("Lighting")while true do if _G.FullBright then C.Brightness=2 C.ClockTime=14 C.FogEnd=100000 C.GlobalShadows=false C.Ambient=Color3.fromRGB(255,255,255)C.OutdoorAmbient=Color3.fromRGB(255,255,255)else C.Brightness=1 C.ClockTime=14 C.FogEnd=100000 C.GlobalShadows=true C.Ambient=Color3.fromRGB(0,0,0)C.OutdoorAmbient=Color3.fromRGB(0,0,0)end
task.wait(1)end end)
y.Main:AddSection("Movement")y.Main:AddToggle("BoatFly",{Title="Enable Boat Flight",Default=false,Callback=function(v)_G.BoatFly=v end})y.Main:AddSlider("Speed",{Title="Drive Power",Default=100,Min=0,Max=1000,Rounding=0,Callback=function(v)_G.BoatSpeed=v end})
y.Main:AddSlider("Height",{Title="Height",Default=20,Min=20,Max=1000,Rounding=0,Callback=function(v)_G.BoatHeight=v end})
...
local H=Instance.new("Frame")H.Parent=G H.Size=UDim2.new(0,400,0,200)H.Position=UDim2.new(0.5,-200,0.4,-100)H.BackgroundColor3=Color3.fromRGB(20,20,20)H.BorderColor3=Color3.fromRGB(255,50,50)H.BorderSizePixel=3
...
local H=Instance.new("Frame")H.Parent=G H.Size=UDim2.new(0,450,0,400)H.Position=UDim2.new(0.5,-225,0.4,-200)H.BackgroundColor3=Color3.fromRGB(20,20,20)H.BorderColor3=Color3.fromRGB(0,255,0)H.BorderSizePixel=3
...
local Q=Instance.new("Frame")Q.Parent=M Q.Size=UDim2.new(1,-10,0,45)Q.BackgroundColor3=Color3.fromRGB(40,40,40)Q.BorderSizePixel=0
...
local T=Instance.new("TextLabel")T.Parent=Q T.Size=UDim2.new(0,250,0,20)T.Position=UDim2.new(0,40,0,23)T.Text="ID: "..q.UserId T.TextColor3=Color3.fromRGB(150,150,150)T.Font=Enum.Font.Gotham T.TextSize=13 T.BackgroundTransparency=1 T.TextXAlignment=Enum.TextXAlignment.Left
end
end
...
K.MouseButton1Click:Connect(function()G:Destroy()end)
local V=Instance.new("TextLabel")V.Parent=H V.Size=UDim2.new(0,200,0,25)V.Position=UDim2.new(0.5,-100,1,-70)V.Text="عدد الضحايا: "..#g.." 👿"V.TextColor3=Color3.fromRGB(255,255,0)V.Font=Enum.Font.GothamBold V.TextSize=16 V.BackgroundTransparency=1
end
...
if e and d~=f then t:Notify({Title="Spy Hub",Content="Already Active!",Duration=3})return end
...
if d==b then e=true t:Notify({Title="Success!",Content="Spy Hub V42 Activated!",Duration=5})x()else t:Notify({Title="Error!",Content="Incorrect Key!",Duration=5})end end})
